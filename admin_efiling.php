...
<body>
                    ...
                    <form id="efilingForm" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="province" id="province" required onchange="updateCourtStations()">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <select name="court_station" id="court_station" required>
    ...
    <style>
        ...
        .efiling-submission-card { 
            position: relative;
        }
        
        .efiling-submission-card::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            height: 3px;
            background: linear-gradient(90deg, #7C0F2F 0%, #7C0F2F var(--progress-width, 20%), #e9ecef var(--progress-width, 20%));
        ...
        .history-card .card-body { padding:0 12px 8px 12px; }
        ...
        .form-group input, .form-group select, .form-group textarea { width:100%; padding:8px 12px; border:1px solid #d6d9de; background:#fafbfc; border-radius:8px; font-size:14px; box-shadow: inset 0 1px 2px rgba(16,24,40,0.04); }
        ...
        .form-group input.error { border-color:#dc3545; box-shadow:0 0 0 4px rgba(220,53,69,.12); }
        .form-group input.valid { border-color:#28a745; box-shadow:0 0 0 4px rgba(40,167,69,.12); }
        ...
        /* Ensure sidebar gets blurred when modal is open */
        body.modal-open {
            overflow: hidden;
        }
    ...
    <script>
        ...
        function generateConfirmationContent() {
            ...
            const content = `
                ...
                <div class="confirmation-section">
                    ...
                    <div class="confirmation-field">
                        <div class="confirmation-label">Case Category:</div>
        ...
        function validateAllSteps() {
            ...
            if (!caseCategory || !caseType || !caseNumber || !caseTitle) {
        ...
        function updateProgress() {
            ...
            const efilingSubmissionCard = document.querySelector('.efiling-submission-card');
            ...
            // Update submission card progress bar
            if (efilingSubmissionCard) {
                efilingSubmissionCard.style.setProperty('--progress-width', `${progressPercentage}%`);
            }
        ...
        function validateCurrentStep() {
            ...
            if (currentStep === 1) {
                ...
                const caseNumber = document.getElementById('case_number').value;
                const caseTitle = document.getElementById('case_title').value;
                nextBtn3.disabled = !(caseCategory && caseType && caseNumber && caseTitle);
                ...
                let isValid = feeExemption;
                
                // If fee exemption is "Yes", condition is required
                if (feeExemption === 'Yes') {
                    isValid = isValid && feeCondition;
                }
                
                nextBtn5.disabled = !isValid;
            }
        }
        ...
        function updateCourtDetails() {
            ...
            if (courtLevel === 'Court of Tax Appeals') {
                ...
                `;
                // Change Court Station to text input for Courts of Appeals
                const courtStationSelect = document.getElementById('court_station');
                const courtStationInput = document.createElement('input');
                courtStationInput.type = 'text';
                courtStationInput.name = 'court_station';
                courtStationInput.id = 'court_station';
                courtStationInput.placeholder = 'Enter Court Station/Office';
                courtStationInput.required = true;
                courtStationInput.style.cssText = 'width:100%; padding:8px 12px; border:1px solid #d6d9de; background:#fafbfc; border-radius:8px; font-size:14px; box-shadow: inset 0 1px 2px rgba(16,24,40,0.04);';
                courtStationSelect.parentNode.replaceChild(courtStationInput, courtStationSelect);
                ...
                province.innerHTML = `
                    ...
                    <option value="Tawi-Tawi">Tawi-Tawi</option>
                `;
                courtStation.innerHTML = '<option value="">Enter Court Station/Office</option>';
                // Change Court Station to text input
                const courtStationSelect = document.getElementById('court_station');
                const courtStationInput = document.createElement('input');
                courtStationInput.type = 'text';
                courtStationInput.name = 'court_station';
                ...
                province.innerHTML = `
                    ...
                    <option value="Tawi-Tawi">Tawi-Tawi</option>
                `;
                // Change Court Station to text input for 2nd Level Court
                const courtStationSelect = document.getElementById('court_station');
                const courtStationInput = document.createElement('input');
                courtStationInput.type = 'text';
                courtStationInput.name = 'court_station';
                ...
                province.innerHTML = `
                    ...
                    <option value="Tawi-Tawi">Tawi-Tawi</option>
                `;
                // Change Court Station to text input for 1st Level Court
                const courtStationSelect = document.getElementById('court_station');
                const courtStationInput = document.createElement('input');
                courtStationInput.type = 'text';
                courtStationInput.name = 'court_station';
        ...
        function updateCourtStations() {
            ...
            // Check if the court station element is a text input, convert it to select if needed
            let courtStationSelect;
            if (courtStationElement.tagName === 'INPUT') {
                // Convert input to select element
                courtStationSelect = document.createElement('select');
                courtStationSelect.name = 'court_station';
                courtStationSelect.id = 'court_station';
                courtStationSelect.required = true;
                courtStationElement.parentNode.replaceChild(courtStationSelect, courtStationElement);
            } else {
                courtStationSelect = courtStationElement;
            }
        ...
        function openCaseSelectionModal() {
            ...
            <?php foreach ($cases as $c): ?>
            {
                ...
                if (!groupedCases[caseType]) groupedCases[caseType] = [];
                groupedCases[caseType].push({
                    id: <?= $c['id'] ?>,
        ...
        function filterCasesInModal() {
            ...
            caseItems.forEach(item => {
                ...
                const caseType = item.getAttribute('data-type').toLowerCase();
        ...
        function clearCaseSelection() {
            ...
            document.getElementById('case_selected_display').textContent = '— No case —';
            document.getElementById('case_id_input').value = '';
            closeCaseSelectionModal();
        }
        
        function confirmCaseSelection() {
            if (window.selectedCase) {
                document.getElementById('case_selected_display').textContent = `#${window.selectedCase.id} — ${window.selectedCase.title}`;
                document.getElementById('case_id_input').value = window.selectedCase.id;
            }
            closeCaseSelectionModal();
        }
